...
test('globe far', async ({ page }, testInfo) => {
  ...
  await expect(page).toHaveScreenshot('globe-far.png', { maxDiffPixelRatio: 0.005 })
})
...
test('euphrates close', async ({ page }, testInfo) => {
  ...
  await expect(page).toHaveScreenshot('euphrates-close.png', { maxDiffPixelRatio: 0.005 })
})
...
test('panel open', async ({ page }, testInfo) => {
  ...
  await expect(page).toHaveScreenshot('panel-open.png', { maxDiffPixelRatio: 0.005 })
})
...
test('story step', async ({ page }, testInfo) => {
  ...
  await expect(page).toHaveScreenshot('story-step.png', { maxDiffPixelRatio: 0.005 })
})
...
test('reduced motion legend', async ({ page }, testInfo) => {
  ...
  await expect(page).toHaveScreenshot('reduced-motion-legend.png', { maxDiffPixelRatio: 0.005 })
})
